...
class NicePreloader extends GameScreen
{
	...
	public function new() 
	{
		...
		var mesh = new Mesh( geom, new MeshBasicMaterial( { side: Side.DoubleSide, color: PLAY_BUTTON_COLOR } ) );
		...
		progressBarBackground = new Mesh( geom,  new MeshBasicMaterial( { side: Side.DoubleSide, color: BAR_BACKGROUND } ) );
		...
		// PROGRESS BAR
		geom = new CircleGeometry( BAR_HEIGHT, 10 );
		progressBar = new Mesh( geom,  new MeshBasicMaterial( { side: Side.DoubleSide, color: BAR_COLOR } ) );
		...
		progress.change.connect( function ( n, p) {
				n = Math.max(n, p);
				for ( o in offsets )
					progressBar.geometry.vertices[o.index].x = n * BAR_WIDTH + o.offset;
					
					
				progressBar.geometry.verticesNeedUpdate = true;
