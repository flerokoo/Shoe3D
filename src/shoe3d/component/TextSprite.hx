		...
		for ( gi in 0..._glyphs.length ) {
			...
			if ( lastUUID == null || lastUUID != gl.page.texture.uuid ) {
				...
				if ( lastPairIndex >= _pairs.length ) {
					var mat = new MeshBasicMaterial( { map: gl.page.texture, transparent: true, side: Side.DoubleSide } );
					pair = { 
						mesh: new Mesh( new Geometry(), mat ), 
						mat: mat,
						num: 0
						};
					if ( owner != null ) owner.transform.add( pair.mesh );
					_pairs.push( pair );
			...
			geom.faces.push( new Face3(4*gi, 4*gi + 1, 4*gi + 2, new Vector3(0, 0, 1) ) );
			geom.faces.push( new Face3(4*gi + 1, 4*gi + 3, 4*gi + 2, new Vector3(0, 0, 1)));
	...
	public function updateLayout() {
		...
		for ( pair in _pairs ) {
			...
			for ( n in 0...pair.num ) {
				...
				trace( "------" + String.fromCharCode( gl.charCode ) );
				trace( gl.y );
				trace( gl.yOffset );
